...
export function ReceiptPDF({ elementId, data }) {
  ...
  ...data?.inv_data.map((customer) => {
    return [
      ...
      {
        text: customer?.buyer_name,
        style: "tableDetail",
      },
      {
        text: customer?.terms_of_payment,
        style: "tableDetail",
      },
      {
        text: customer?.due_date,
        style: "tableDetail",
      },
    ];
  })
  ...
  var docDefinition = {
    ...
    content: [
      ...
      //SUB DETAILS
      {
        text: 'ใบเสร็จรับเงินฉบับน้ีจะมีผลสมบูรณ์เมื่อเช็คของท่านเรียกเก็บเงินจากธนาคารเรียบร้อยแล้ว"\n\n This Receipt will become valid only when cheque in payment has been cleared by the bank.',
        fontSize: 8,
        margin: [0, 5, 0, 10],
      },
      //END OF SUB DETAILS
