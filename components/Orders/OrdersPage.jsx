...
export default function OrdersPage(
  ...
  const handleSelectPageSize = (sizeValue) => {
    getOrders({
      url: `/api/orders?&orderCode=${orderCode}&status=${status}&startDate=${startDate}&endDate=${endDate}&page=1&pageSize=${sizeValue}`,
    });
  };
    ...
    <div fluid className="pt-4 ">
      <Card className="text-center rounded shadow p-4">
        <div className="d-flex align-items-center justify-content-between mb-4">
          <Card.Title className="mb-0">รายการสั่งซื้อ</Card.Title>
          <OrderAddModal getData={getOrders} />
        </div>
        <MyTable
